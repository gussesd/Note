...
function P2pPathData:ShowUnit2RandomPoint(data, p2pPotIndex, index, instGroupCfg, showInfo,timeOff)
    ...
    local pos, points, exOffset = self:CalculateChildPosition(index, instGroupCfg, p2pPotIndex)
    local infoData = {
        cfg = data,
        id = data.id,
        originalGroup = instGroupCfg.id,
        pos = pos,
...
---@ 根据队列判断计算排列并针对偏移量计算移动路点,同时也算出额外路径点的offset
function P2pPathData:CalculateChildPosition(index, data, p2pPotIndex)
    ...
    local exOffset =  self:SetPos2Layer(index, data, true)
    return initPos, points, exOffset
end
...
---@ 设计布局排版
function P2pPathData:SetPos2Layer(index, data, force)

    local rowCount = 1
    local columCount = data.constraint_type[2] -- 行数

    if data.constraint_type[1] == 0 and not force then
        -- 排列方式
        return { 0, 0, 0 }
    else
...
function P2pPathData:RecycleUnit(instGroupId)
    ...
    if self.instGroupList[groupId] then
        ...
        if #self.instGroupList[groupId].units == 1 then
            ...
            -- 多个人,需要等待所有人员一起结束后销毁并创建
            local need = self:WaitGroupRecycle(groupId)
            if need then
                self.instGroupList[groupId] = nil
                table.insert(self.tempIndexArray, groupId) --重新注入随机队列
                self:NextShowUnit(groupId)
...
---@ 等待组员的所有人员都结束
function P2pPathData:WaitGroupRecycle(groupId)
    -- 统计组员累计回收次数
    local recycleCount = self.instGroupList[groupId].waitRecycle  or 0
    recycleCount  = recycleCount + 1
    self.instGroupList[groupId].waitRecycle = recycleCount
    if recycleCount == #self.instGroupList[groupId].units then
        return true
    end
    return false
end
